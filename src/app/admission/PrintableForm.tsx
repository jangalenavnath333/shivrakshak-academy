'use client'
import type { AdmissionFormDetails } from '@/types'
...
export type FormValues = AdmissionFormDetails
...
function PrintableForm({ form, photo, rollNumber }: Props) {
  const f = form
  ...
  return (
    <div className="print-root" style={{ position: 'fixed', left: -99999, top: 0, zIndex: -1, width: 794, background: '#fff' }}>
      ...
      <div className="pdf-page" style={PAGE}>
        <div style={{ border: '2.2px solid #000', minHeight: 1050, padding: '22px 30px 20px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ fontSize: 16.5, fontWeight: 700, lineHeight: 2.6 }}>
            ...
            <DotLine label="भरलेली रक्कम:-" value={f.paidAmount ? `₹ ${Number(f.paidAmount).toLocaleString('en-IN')} /-` : ''} />
            <DotLine label="बाकी रक्कम:-" value={f.totalFee ? `₹ ${Math.max(0, Number(f.totalFee) - Number(f.paidAmount || 0)).toLocaleString('en-IN')} /-` : ''} />
